...
class DAIPipeline(StableDiffusionControlNetPipeline):
    ...
    def __init__(
        ...
        prompt = "remove moire pattern, high quality, detailed",
        ...
        self.control_image_processor = MarigoldImageProcessor(
            vae_scale_factor=self.vae_scale_factor
        )
    ...
    def check_inputs(
        ...
        if num_inference_steps is None:
            raise ValueError(
                "`num_inference_steps` is not specified and could not be resolved from the model config."
            )
        ...
        if processing_resolution % self.vae_scale_factor != 0:
            raise ValueError(
                f"`processing_resolution` must be a multiple of {self.vae_scale_factor}."
            )
        if resample_method_input not in (
            "nearest",
            "nearest-exact",
            "bilinear",
            "bicubic",
        ...
        if resample_method_output not in (
            "nearest",
            "nearest-exact",
            "bilinear",
            "bicubic",
        ...
        if ensembling_kwargs is not None:
            ...
            if "reduction" in ensembling_kwargs and ensembling_kwargs[
                "reduction"
            ] not in ("closest", "mean"):
                raise ValueError(
                    "`ensembling_kwargs['reduction']` can be either `'closest'` or `'mean'`."
                )
        ...
        for i, img in enumerate(image):
            if isinstance(img, np.ndarray) or torch.is_tensor(img):
                if img.ndim not in (2, 3, 4):
                    raise ValueError(
                        f"`image[{i}]` has unsupported dimensions or shape: {img.shape}."
                    )
                H_i, W_i = img.shape[-2:]
                N_i = 1
                if img.ndim == 4:
        ...
        if latents is not None:
            ...
            if latents.dim() != 4:
                raise ValueError(
                    f"`latents` has unsupported dimensions or shape: {latents.shape}."
                )
            ...
            if processing_resolution > 0:
                ...
                if new_H == 0 or new_W == 0:
                    raise ValueError(
                        f"Extreme aspect ratio of the input image: [{W} x {H}]"
                    )
                W, H = new_W, new_H
            w = (W + self.vae_scale_factor - 1) // self.vae_scale_factor
            h = (H + self.vae_scale_factor - 1) // self.vae_scale_factor
            shape_expected = (
                num_images * ensemble_size,
            ...
            if latents.shape != shape_expected:
                raise ValueError(
                    f"`latents` has unexpected shape={latents.shape} expected={shape_expected}."
                )
        ...
        if generator is not None:
            if isinstance(generator, list):
                ...
                if not all(
                    g.device.type == generator[0].device.type for g in generator
                ):
                    raise ValueError(
                        "`generator` device placement is not consistent in the list."
                    )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not skip_preprocess:
            ...
            print('shape after pre: ', image.shape[2:])
        else:
            padding = (0, 0)
            original_resolution = image.shape[2:]
            image = self.image_processor.load_image_canonical(image, device, dtype)
            image = image * 2.0 - 1.0
        ...
        # prediction = self.vae.decode(latent_x_t / 0.18215).sample
    ...
    def prepare_latents(
        ...
        image_latent = image_latent.repeat_interleave(
            ensemble_size, dim=0
        )  # [N*E,4,h,w]
    ...
    def decode_prediction(
        ...
        if (
            pred_latent.dim() != 4
            or pred_latent.shape[1] != vae_2.config.latent_channels
        ):
            raise ValueError(
                f"Expecting 4D tensor of shape [B,{vae_2.config.latent_channels},H,W]; got {pred_latent.shape}."
            )

        prediction = vae_2.decode(
            pred_latent / vae_2.config.scaling_factor,
            skip_connections,
            return_dict=False,
        )[
            0
    ...
    def ensemble_normals(
        ...
        mean_normals = MarigoldNormalsPipeline.normalize_normals(
            mean_normals
        )  # [1,3,H,W]
