...
class CustomAutoencoderKL(AutoencoderKL):
    ...
    def encode(self, x: torch.Tensor, return_dict: bool = True):
        ...
        posterior, skip_connections = self.encoder(x)
    ...
    def forward(
        ...
        skip_connections = self.encode(sample, return_dict=True)
        ...
        # Decode the latent representation with skip connections
        dec = self.decode(pred_latent, skip_connections, return_dict=return_dict)
...
class CustomEncoder(Encoder):
    ...
    def forward(self, sample: torch.Tensor) -> list[torch.Tensor]:
        ...
        for i, (down_block, zero_conv) in enumerate(
            zip(self.down_blocks, self.zero_convs)
        ):
            # import pdb; pdb.set_trace()
            sample = down_block(sample)
            if i != len(self.down_blocks) - 1:
                sample_out = nn.functional.interpolate(
                    zero_conv(sample),
                    scale_factor=2,
                    mode="bilinear",
                    align_corners=False,
